...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			int x1 = r.getSource().getX();
			int y1 = r.getSource().getY();
			int x2 = r.getDestination().getX();
			...
			int idx = r.getDestination().getCurrentGreen();
			if (idx != -1 && r.equals(r.getDestination().getRoadIn().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0, (double) r.getTotalContamination() / (1.0 + (double) r.getContaminationLimit())));
